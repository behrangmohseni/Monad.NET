...
|----|-------|----------|-------------|
| MNT001 | Unchecked Unwrap | Warning | Detects `Unwrap()` without checking `IsSome`/`IsOk` first |
...
| MNT011 | Unwrap in LINQ | Warning | Using `Unwrap` inside LINQ can throw unexpectedly |
...
| MNT003 | Map+GetOrElse | Info | `Map().GetOrElse()` can be `Match()` or `MapOr()` |
| MNT004 | FlatMap to Map | Info | `FlatMap(x => Some(f(x)))` can be `Map(f)` |
...
### MNT001: Unchecked Unwrap

```csharp
// Warning: Option should be checked before calling Unwrap()
...
// Better:
var value = option.Expect("No value");
```
...
### MNT003: Map+GetOrElse

```csharp
// Info: Can be simplified
var result = option.Map(x => x.ToString()).GetOrElse("default");
...
### MNT004: FlatMap to Map

```csharp
// Info: FlatMap returning Some can be Map
var result = option.FlatMap(x => Option<int>.Some(x * 2));
...
var value = option.Unwrap(); // I checked elsewhere
