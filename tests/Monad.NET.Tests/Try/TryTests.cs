...
public class TryTests
{
    ...
    {
        Assert.Throws<ArgumentNullException>(() => Try<string>.Ok(null!));
    }

    [Fact]
    public void Failure_WithNull_ThrowsException()
    {
        Assert.Throws<ArgumentNullException>(() => Try<int>.Error(null!));
    }
    ...
    {
        var tryValue = Try<int>.Error(new Exception("error"));
...
#pragma warning restore CS0162
    ...
    {
        var tryValue = Try<int>.Ok(42);
        var result = tryValue.Bind(x => Try<string>.Ok(x.ToString()));
    ...
    {
        var try1 = Try<int>.Ok(42);
        var try2 = Try<string>.Ok("hello");
    ...
    public void Zip_FirstFailure_ReturnsFirstException()
    {
        ...
        var try1 = Try<int>.Error(exception);
        var try2 = Try<string>.Ok("hello");
    ...
    public void Zip_SecondFailure_ReturnsSecondException()
    {
        ...
        var try1 = Try<int>.Ok(42);
        var try2 = Try<string>.Error(exception);
    ...
    {
        var try1 = Try<int>.Ok(10);
        var try2 = Try<int>.Ok(20);
    ...
    {
        var tryValue = Try<int>.Error(new Exception("error"));
        var recovered = tryValue.RecoverWith(ex => Try<int>.Ok(100));
    ...
    public void TapFailure_OnFailure_ExecutesAction()
    {
        ...
        var result = tryValue.TapError(ex => executed = true);
    ...
    {
        var nested = Try<Try<int>>.Ok(Try<int>.Ok(42));
    ...
    public void Flatten_OuterFailure_ReturnsFailure()
    {
        ...
        var nested = Try<Try<int>>.Error(exception);
        var flattened = nested.Flatten();
    ...
    {
        var exception = new Exception("error");
        var result = Result<int, Exception>.Error(exception);
        var tryValue = result.ToTry();
