...
- **default(Result<T,E>)** (v2.0): Never use `default(Result<T,E>)` — it throws `InvalidOperationException` on any operation. Always use `Result<T,E>.Ok(value)` or `Result<T,E>.Err(error)` factory methods.
